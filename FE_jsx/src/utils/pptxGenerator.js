...
export async function generatePPTXFromData(presentationData) {
  ...
  for (const slideData of presentationData.slides) {
    ...
    const hasImage = slideData.image_url || slideData.image_path;
    ...
    // Add image if exists
    if (hasImage) {
      const imageUrl = slideData.image_url || slideData.image_path;
...
export async function generateSlidePreview(slideData) {
  // This would require additional libraries for rendering
  // For now, return the slide's image or null
  return slideData.image_url || slideData.image_path || null;
}
